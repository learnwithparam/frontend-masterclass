#!/usr/bin/env bash
# Module 09: Smoke Test — Optimistic UI & Streaming (requires Docker)
...
echo -e "${CYAN}Module 09: Optimistic UI & Streaming — Smoke Test${NC}"
echo ""
...
# ── Test 2: Auth flow ───────────────────────────────────────────────
TOKEN=$(register_and_login "smokeuser09_$(date +%s)" "password123" "$API")
