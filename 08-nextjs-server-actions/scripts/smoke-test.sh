#!/usr/bin/env bash
# Module 08: Smoke Test — Next.js Server Actions (requires Docker)
...
echo -e "${CYAN}Module 08: Next.js Server Actions — Smoke Test${NC}"
echo ""
...
# ── Test 2: Auth flow ───────────────────────────────────────────────
TOKEN=$(register_and_login "smokeuser08_$(date +%s)" "password123" "$API")
